...
@ManagedBean
@ViewScoped
public class EditTeamsOfProjectBean2 {
	...
	public EditTeamsOfProjectBean2() {
	...
	public void updateFields() {
		...
		String[] split = team.split("#");
		 List<Team> teamList = teamDAO.readTeamById(Integer.parseInt(split[1]));
		 Team currentTeam = teamList.get(0);
		
		studentsTarget.clear();
		for (int index = 0; index < currentTeam.getStudents().size(); index++) {
			studentsTarget.add(currentTeam.getStudents().get(index).getFirstname()+" "+currentTeam.getStudents().get(index).getLastname()+" #"+currentTeam.getStudents().get(index).getId());
			if (studentsSource.contains(currentTeam.getStudents().get(index).getFirstname()+" "+currentTeam.getStudents().get(index).getLastname()+" #"+currentTeam.getStudents().get(index).getId())) {
				studentsSource.remove(currentTeam.getStudents().get(index).getFirstname()+" "+currentTeam.getStudents().get(index).getLastname()+" #"+currentTeam.getStudents().get(index).getId());
			}
		}
		studentListModel = new DualListModel<String>(studentsSource, studentsTarget);
	}
